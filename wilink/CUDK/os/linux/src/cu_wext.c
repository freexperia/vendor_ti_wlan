...
{
	TCuWext* pCuWext = (TCuWext*)hCuWext;
	S32 res = -1, NumberOfItems;
	U32 buffer_size = IW_SCAN_MAX_DATA;
	U8* buffer;

	NumberOfItems = 0;
	pCuWext->req_data.data.flags = 0;

	/* increase buffersize until scan result
	   fits or buffer is 32kB (more than enough).
	   if result is larger than 32kB then get BSSID
	   will fail. */
	while (res != OK && buffer_size <= 0x8000)
	{
		buffer = os_MemoryCAlloc(buffer_size, sizeof(U8));
		if (buffer == NULL)
		{
		...
		pCuWext->req_data.data.length = buffer_size;
		...
		if (res != OK)
		{
			/* free current buffer and double buffer-
			   size. */
