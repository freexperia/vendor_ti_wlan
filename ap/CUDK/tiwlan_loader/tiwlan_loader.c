...
static const char AP_DRIVER_PROP_NAME[]    = WIFI_AP_DRIVER_PROP_NAME;
                 ...
                 PS8 init_file_name, PS8 firmware_file_name )
{
	...
	os_error_printf(CU_MSG_INFO1, (PS8)"| wlan_ap_loader: initializing |\n");
	os_error_printf(CU_MSG_INFO1, (PS8)"+---------------------------+\n");
...
#ifdef ANDROID
				check_and_set_property(AP_DRIVER_PROP_NAME, "failed");
#endif
				return -1;
			}
		}
...
#ifdef ANDROID
	check_and_set_property(AP_DRIVER_PROP_NAME, "ok");
	release_wake_lock(PROGRAM_NAME);
#endif
	return 0;
}
